...
export default function Products() {
    ...
    const getProductsLists = async () => {
        setLoading(true)
        try {
            const { code, info, total } = await getAllProducts(pageNum, 10, dateFilter)
            if (code === 200) {
                setProductsData([...productsData, ...info?.data])
            }
        } catch (error) {
            console.log(error)
        }
    ...
    const handleDateChange = (e, formatedDate) => {
    ...
    return (
        <div>
            <div class='flex justify-end items-center mt-5 pr-5'>
                <Datepicker handleDateChange={handleDateChange} value={dateFilter} />
            </div>

            {
                productsData.length > 0 ? <div className='grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-4'>
                    {
                        productsData?.map((product, i) => {
                            const { } = product;
                            return i === productsData.length - 1 &&
                                !loading &&
                                pageNum <= 10 ? (
